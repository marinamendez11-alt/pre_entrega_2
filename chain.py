...
def construir_modelo() -> ChatOpenAI:
    """Instancia el modelo base. Reutiliza la misma idea de configuración
    centralizada que vimos en el Módulo 1 (Unified Async LLM Client)."""
    return ChatOpenAI(
...
async def process_text(texto: str) -> EntidadesTecnicas:
    ...
    try:
        ...
        # Si con_retry() agota los 3 intentos y sigue fallando (JSON
        # incompleto por corte de tokens, validación de Pydantic, etc.),
        # se loguea el fallo de forma controlada en vez de dejar que
        # la excepción se propague sin contexto.
